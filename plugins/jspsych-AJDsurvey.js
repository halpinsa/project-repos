 ...
 *
 * Shane Martin (amended by Alasdair J. Dalgarno)
 *
...
jsPsych.plugins['AJDsurvey'] = (function() {
  ...
  plugin.trial = function(display_element, trial) {
    ...
    $trial_form.append($('<div>', {
      "id": preamble_id_name,
      "class": preamble_id_name + " jumbotron"
    }));
    ...
    for (var i = 0; i < trial.questions.length; i++) {
      ...
      $trial_form.append($('<div>', {
        ...
        "class": question_classes.join(' ') + " card"
      }));
      ...
      $(question_selector).append(
        '<p class="' + plugin_id_name + '-text survey-multi-choice card-header">' + trial.questions[i] + '</p>'
      );
      ...
      for (var j = 0; j < trial.options[i].length; j++) {
        ...
        // Create function to enable button
        $(option_id_selector).click(function(){
          var numQuestions = trial.questions.length;
          var checked = 0;
          ...
          $('input:radio').each(function() {
            ...
            if (numQuestions === checked){
              $('#'+ plugin_id_name + '-next').prop('disabled', false);
            }
          });
        });
    ...
    $('#'+ plugin_id_name + '-next').prop('disabled', true)
